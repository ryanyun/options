...
function OptionContract (contractId) {
  ...
  // this.privkey = "f01c0b4098c0a74842264d772d59ea890d4f91320f3cce74d84554a28e68c887";
  this.userAddress = "d2600f116b6c47926ee1f954959506710f50a8f9";
  // this.userAddress = "53a2f9f53e0a223b7c19bb43704c8fbbac9de52d";
}
...
OptionContract.prototype.createOption = function(sellerAddress, buyerAddress,amount,expirationTimeInSeconds){
  ...
  var etherGas = Units.ethValue(2).in("ether");
  self.contract.state.createOption.apply(null,args).txParams({value : etherGas}).callFrom(self.privkey).then(function(result){
      self.storageAfterTX(result).then(function (contractData) { 
        self.sendContractData(contractData);
      });
    });
};
...
OptionContract.prototype.getData = function(){
  ...
  var args = [];
  var etherGas = Units.ethValue(2).in("ether");
  self.contract.state.getAssetAmount.apply(null,args).txParams({value : etherGas}).callFrom(self.privkey).then(function(result){
      self.storageAfterTX(result).then(function (contractData) { 
        self.sendContractData(contractData);
      });
    });
...
OptionContract.prototype.sendContractData = function(contractData){
  var self = this;
  $.ajax({
    type: 'PUT',
    url: "/api/contracts/"+self.contractId,
    data: { "contract": { "address": contractData.self} },
    success: function() {
